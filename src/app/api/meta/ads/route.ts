...
export async function GET(request: Request) {
    try {
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: error.error?.message || "Erro ao buscar anúncios." }, { status: 200 });
        }
        ...
        return NextResponse.json({ connected: true, ads: adsWithInsights, total: ads.length });
    } catch (error: any) {
